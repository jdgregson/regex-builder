...
(function() {
  ...
  on_window_resize = function(singleCallback) {
    update_affix();
    if(!singleCallback) {
      self.setTimeout(() => {on_window_resize(true)}, 100);
    }
    $('#exp_dsp').width($('#exp').outerWidth()-4);
    resizeWiki();

    if ($window.width() < 768) {
      $('#exp_col').removeClass('col-xs-10').addClass('col-xs-12');
      $('#flags_col').removeClass('col-xs-2').addClass('col-xs-12');
      $('#flags_col').addClass('responsive');
      $('#flags').addClass('responsive');
    } else {
      $('#exp_col').removeClass('col-xs-12').addClass('col-xs-10');
      $('#flags_col').removeClass('col-xs-12').addClass('col-xs-2');
      $('#flags_col').removeClass('responsive');
      $('#flags').removeClass('responsive');
    }
  };
...
function openModalDialog(title, content, hide = false, customClass = '') {
  let html = `
  <div class="dialog-wrap ${customClass}"
      onclick="closeModalDialog(event, this, ${hide})">
    <div class="dialog-table">
      <div class="dialog-cell" id="dialog-backdrop">
        <div class="dialog-box">
          <label class="dialog-title">${title}</label>
          <label class="dialog-close-button">×</label>
  ...
  opener.innerHTML = html;
  document.body.appendChild(opener.children[0]);
...
function closeModalDialog(e, dialog, hide = false) {
  if(e.target.className === 'dialog-cell' ||
      e.target.classNname === 'dialog-backdrop' ||
      e.target.className === 'dialog-close-button') {
    if(hide) {
      $('#hidden').append(
        $(dialog.querySelector('.dialog-content').children).detach()
      );
...
function openWiki() {
  let selectedTheme = document.getElementById('theme_select').value;
  let storedTheme = localStorage.theme_select;
  let appliedTheme = selectedTheme !== storedTheme? selectedTheme:storedTheme;
  openModalDialog('RegEx Wiki (from MDN)', '<iframe src="wiki/index.html?' +
    `theme=${appliedTheme}" id="wiki-iframe"></iframe>`, false, 'wiki-modal');
  self.setTimeout(() => {
    let dialog = document.getElementsByClassName('dialog-box')[0];
    let iframe = document.getElementById('wiki-iframe');
...
function resizeWiki() {
  let modal = document.querySelector('.wiki-modal .dialog-content');
  let title = document.querySelector('.wiki-modal .dialog-title');
  let iframe = document.querySelector('.wiki-modal iframe');
  if(modal) {
    let parentHeight = parseFloat(getComputedStyle(modal.parentElement).height);
    let titleHeight = parseFloat(getComputedStyle(title).height);
    let offsetHeight = parentHeight - titleHeight - 15;
    console.log(`${offsetHeight}px`);
...
function applyTheme() {
  ...
  document.getElementById('theme-style').setAttribute('href',
    `styles/theme-${appliedTheme}.css`);
}

window.addEventListener('load', () => {
  let settingsOpenButton = document.getElementById('open-settings-button');
  settingsOpenButton.addEventListener('click', () => {
    openModalDialog('Settings', $('#settings').detach(), true);
  });
